...
struct SessionDataServiceTests {
    ...
    @MainActor func testCreateSession() async throws {
    ...
    @MainActor func testUpdateSession() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func testDeleteSession() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func fetchAllSessionsSortedByDate() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func fetchSessionsByDateRange() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func filterSessionsByTreatmentType() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func testSaveDraft() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func testClearDraft() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func draftPersistenceAcrossInstances() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
    ...
    @MainActor func validSessionValidation() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func futureDateValidation() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func invalidMoodScaleValidation() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func testForceSave() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func backgroundSave() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func performanceWithManySession() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
    ...
    @MainActor func emptyDatabaseHandling() async throws {
        // Create isolated test environment
        let config = ModelConfiguration(isStoredInMemoryOnly: true)
        let container = try ModelContainer(for: TherapeuticSession.self, configurations: config)
        let service = SessionDataService(modelContext: container.mainContext)
